...
namespace LocalServicesDirectory.Web.Services
{
    public class ServiceApiClient
    {
        ...
        public Task<List<CategoryDto>> GetCategoriesAsync()
            ...
            => Try(async () => await _http.GetFromJsonAsync<List<CategoryDto>>("api/categories") ?? new(),
                   new List<CategoryDto>());

        public Task<bool> CreateCategoryAsync(CreateOrUpdateCategoryDto dto)
            => Try(async () => (await _http.PostAsJsonAsync("api/categories", dto)).IsSuccessStatusCode);

        public Task<bool> UpdateCategoryAsync(Guid id, CreateOrUpdateCategoryDto dto)
            => Try(async () => (await _http.PutAsJsonAsync($"api/categories/{id}", dto)).IsSuccessStatusCode);

        public Task<bool> DeleteCategoryAsync(Guid id)
            => Try(async () => (await _http.DeleteAsync($"api/categories/{id}")).IsSuccessStatusCode);

        
        public Task<List<CityDto>> GetCitiesAsync()
            => Try(async () => await _http.GetFromJsonAsync<List<CityDto>>("api/cities") ?? new(),
                   new List<CityDto>());
        ...
        public Task<bool> CreateCityAsync(CreateOrUpdateCityDto dto)
            => Try(async () =>
            {
                ...
                var resp = await _http.PostAsJsonAsync("api/cities", new { name = dto.Name.Trim() });
        ...
        public Task<bool> UpdateCityAsync(Guid id, CreateOrUpdateCityDto dto)
            => Try(async () => (await _http.PutAsJsonAsync($"api/cities/{id}", new { id, name = dto.Name.Trim() })).IsSuccessStatusCode);

        public Task<bool> DeleteCityAsync(Guid id)
            => Try(async () => (await _http.DeleteAsync($"api/cities/{id}")).IsSuccessStatusCode);
        ...
        public Task<ServiceDto?> GetAsync(Guid id)
            => Try(async () => await _http.GetFromJsonAsync<ServiceDto>($"api/services/{id}"),
                   (ServiceDto?)null);
